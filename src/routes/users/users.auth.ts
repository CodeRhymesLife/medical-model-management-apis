...
export class UsersAuthUtils {
    /** Verifies the google id token is valid and returns its payload */
    static async verifyIdToken (req: Request): Promise<GoogleAuthData> {
    ...
    static async getUserFromIdToken (req: Request): Promise<InstanceType<User>> {
        try {
     ...
     */
    static async getUserFromDevEmail (req: Request): Promise<InstanceType<User>> {
     ...
     */
    static async getUser (req: Request): Promise<InstanceType<User>> {
        // If we're running in development mode this will try and
        // find a user based on an email in the header
        const devUser = await UsersAuthUtils.getUserFromDevEmail(req);
        if (devUser) {
            return devUser;
        }
        ...
        return UsersAuthUtils.getUserFromIdToken(req);
    }
    ...
    static async load (req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            req.authedUser = await UsersAuthUtils.getUser(req); // eslint-disable-line  no-param-reassign
    ...
    static async isMaster (req: Request, res: Response, next: NextFunction) {

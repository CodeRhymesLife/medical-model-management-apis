...
parser.add_argument('--modelId', required="true", help="Model id");
parser.add_argument('--models', nargs='+', required="true", help="A list of paths to model files that will be prepared for Unity");
parser.add_argument('--outputDir', required="true", help="The directory where the models will be created");
parser.add_argument('--objsDir', required="true", help="The directory where the objs will be stored");
parser.add_argument('--texturesDir', required="true", help="The directory where the textures will be stored");
parser.add_argument('--filesAreContinuous', dest='filesAreContinuous', action='store_true', help="True if eawch file should be a continuous mesh");
...
def save_images_to_textures_dir():
	...
	for image in bpy.data.images:
		...
		try:
			...
			logging.exception("Failed to pack image. Moving on...");
			continue;

		# change the images path and save
		filename = os.path.basename(image.filepath);
		logging.info("filename: " + filename);
		image.filepath = os.path.join(args.texturesDir, filename);
...
def create_obj_for_each_part ():
	...
	for mesh in meshes:
		if "_collider" not in mesh.name:
			partPath = os.path.join( args.objsDir, mesh.name + ".obj" );
			mesh.select = True;
			bpy.ops.export_scene.obj( filepath=partPath, use_selection=True )
			mesh.select = False;
...
ensure_files_exist(args.models);
...
create_obj_for_each_part();

# Take a picture of the model
take_picture();

# Return data by printing it to the console
print ("DATA_START {} DATA_END".format( json.dumps( modelPartData ) ) )
